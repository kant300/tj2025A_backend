...
# [1] INSERT : 레코드 삽입, "레코드란? 테이블내 행(가로) 단위의 데이터들, 1줄 vs 1행 vs 1줄
# 주의할점 : 테이블내 데이터 삽입시 레코드 단위로 삽입 , 한칸한칸에 데이터삽입이 아닌 줄/행/레코드 단위삽입
...
# (2) auto increment 제약조건의 속성은 생략할 경우 자동번호(1씩증가된) 할당 	# PK에서 주로 사용
insert into test( mcount , mname ) values( 50 , "신동엽" );
# (3) default 제약조건의 속성은 값 생략할 경우 정의된 기본값이 할당
insert into test( mname ) value( "서장훈" );
# (4) not null 제약조건의 속성은 값이 꼭 삽입해야 한다. 
# insert into test( mno , mcount ) value( 5, 40 ); Error Code: 1364. Field 'mname' doesn't have a default value	0.000 sec
# (5) unique 제약조건의 속ㅇ은 중복값일 경우 삽입 불가능 
# insert into test( mname ) values( "유재석" ); Error Code: 1062. Duplicate entry '유재석' for key 'test.mname'	0.000 sec
...
# [2] SELECT : 레코드 조회
# 주의할점 : 조회 결과는 기존 테이블과 상관없다. select만 result table(Result Grid) 존재한다. 조회결과는 항상 테이블/표
...
select * from test where mno = 3;	# 'mno'속성값이 3과 같은 레코드 조회
...
update test set mcount = 0; # Error Code: 1175. You are using safe update mode and you tried to update a table without a WHERE that uses a KEY column.  To disable safe mode, toggle the option in Preferences -> SQL Editor and reconnect.	0.000 sec
# ---> safe mode 끄기를 해야 수정이 가능해진다.
# 2. update 테이블명 set 속성명 = 새로운 값 where 조건절;	: 특정한 레코드의 속성값 수정
