...
-- 즉] ordess 테이블의 총 레코드수 ( count 함수 ) 조회
select * from orders;
select count( order_id ) from orders;   -- 24
select count( * ) from orders;			-- 24
...
-- [문제 3] orders 테이블에서 '김민준' *고객*이 주문한 총 주문 수량(order_qty의 합계)을 조회하세요.
# customer(고객명) 그룹 전 조건을 사용 = where 	vs 	 having
select * from orders;
...
-- [문제 4] books 테이블에서 각 장르(genre)*별*로 도서가 총 몇 권씩 있는지 조회하세요.
select genre , stock from books;
select genre from books group by genre; # 그룹 : ~별 , ~끼리
select count( * ) from books; -- 도서가 총 몇권 == 레코드수 == 도서1개당 == 레코드1개 == 레코드수 구하기
...
-- 총 주문수량이 4권 이상인 고객만 조회하세요. (그룹/집계 후 가능한 조건) = having
...
-- [문제 7] books 테이블에서 *재고(stock)가 5권 미만*인 도서들의 *평균 가격*을 조회하세요.
select avg( price ) as 평균가격 from books where stock < 5;
 

-- [문제 8] orders 테이블에서 3번 이상 주문된 도서의 book_id와 주문 횟수를 조회하세요.
select book_id , count(*) as 주문횟수 from orders 
	group by book_id having 주문횟수 >= 3; -- 레코드수가 3개이상인 도서 = having

-- [문제 9] orders 테이블에서 총 주문 수량이 5권을 초과하는 고객의 customer와 총 주문 수량을 조회하세요.
select customer , sum( order_qty ) from orders group by customer; -- 총 주문수량 = having
select customer , sum( order_qty ) as 총주문수량 from orders 
	group by customer having 총주문수량 > 5;
