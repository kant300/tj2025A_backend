...
# [1] 그룹 : ~별 , ~끼리 , 두개이상의 중복[레코드/행/가로]되는 속성값들의 묶음
...
-- 오류 : bpname 속성값이 동일하더라도 bprice다를수 있기 때문에
...
-- select mid , bamount from buy group by mid;			-- 실행불가
-- select mid , sum( bamount ) from buy;				-- 실행불가
