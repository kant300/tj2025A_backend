...
select mid as 회원아이디 , mname 회원명 from member;	-- as는 생략가능하고 띄어쓰기로 구분
...
select * from member where mnumber = 4;			-- member 테이블의 mnumber속성의 값이 4이면
...
select * from member where mheight <= 162;		-- member 테이블의 mheight속성의 값이 162이하인
...
select * from member where mphone1 = null;	-- 속성값이 null이면 [x 불가능 ]
...
select * from member order by mdebut asc;	-- 데뷔날짜 오름차순
select * from member order by mdebut;	-- asc/desc 생략시 자동으로 asc(기본)
select * from member order by mdebut desc;
# 주의할점 : 2개이상의 정렬은 , (쉼표) 구분하되, 1차 정렬에서 중복값이 있을경우 중복값끼리 2차 정렬
...
# limit 시작레코드번호(0~) , 개수
select * from member limit 2; 	-- 조회결과에서 상위2개만 제한
select * from member limit 0 , 2;	-- 조회결과에서 0번 레코드부터 2개 제한
select * from member limit 2 , 3;	-- 2번 레코드부터 3개 제한
select * from member order by mheight desc limit 3;	-- mheight 속성을 내림차순으로 정렬후 상위3개
