...
public class Example1 {
    public static void main(String[] args) {
        ...
        3. 예외 종류
            1) 일반 예외 : 컴파일(실행)될때 예외 처리 코드여부 검사 예외
                - 입출력 , 네트워크 , 파일 , JBDC(자바-DB연동) , 주로 자바외부 통신 : Class.forName() , Thread.sleep()
            2) 실행 예외 : 컴파일(실행)될때 예외 처리 코드여부 검사 하지 않는 예외 , 개발자의 (경험) 의존도가 크다.
            * 웹/앱 라이브러리/프레임워크( Spring ) 는 자동 예외처리
        4. 예외란
            - 예외/오류 발생시 예외를 고치는게 아니라 **흐름을 제어** vs if 비슷
            - 프로그램을 24시간 중단없이 실행하기 위한 안전한 로직 구현<100% 없다. --> 유지보수>
        5. 예외 클래스
            Exception : 모든 예외들의 상위 클래스(슈퍼클래스)
        ...
        try { // 1. 일반예외 발생 코드를 try{} 감싼다.
            Class.forName("java.lang.String"); // Class.forName( 클래스경로 ); // 해당경로의 클래스를 읽어오는 함수
        ...
        // [4] 실행예외 : 프로그램 실행 도중에 발생하는 예외, <예측>
        // !! NullPointerException : 객체가 없는데 객체에 접근연산자를 사용하면/객체를 호출하면 발생하는 예외
        try {
            String str1 = "강호동";
        ...
        try {
            ...
            int int3 = Integer.parseInt(str3); // Integer.parseInt();  정적메소드 , 문자열타입을 int 타입으로 변환하여
        ...
        // [6] 실행예외 : ArrayIndexOutOfBoundsException : 존재하지 않는 인덱스 호출 예외
        try{
            int[] intArray = { 1 , 2, 3 };
            System.out.println( intArray[0] ); // 0번 인덱스 존재한다.
            System.out.println( intArray[4] ); // 4번 인덱스 존재하지 않는다. 오류발생
        }catch ( ArrayIndexOutOfBoundsException e ) {
            System.out.println("[예외발생6] 없는 인덱스입니다." + e); //[예외발생6] 없는 인덱스입니다.java.lang.ArrayIndexOutOfBoundsException: Index 4 out of bounds for length 3
        }
        ...
        catch ( Exception e4 ) { System.out.println( e4 ); } //위 3개를 한개로 처리가능
        // - 주의할점1 : 다중 catch 시 먼저 발생한 예외부터 하나의 예외발생하고 하나의 catch만 실행된다.
        // - 주의할점2 : Exception클래스는 모든 예외를 처리하므로 다중 catch 시에는 맨 아래에서 정의한다.
