...
public class 실습15 {
    public static void main(String[] args) {
        // * 파일처리 목적 : (저장) 자바외 저장소로 영구저장 가능하다.
        // 문제1 : ( 출력 : 자바 --> 파일 )
        // (1) 파일 경로 지정
        String path = "src/day16/diary.txt";
        try { // try{ 예외가 발생할 것  같은 또는 일반예외}
            // (2) 출력 객체 생성 , 일반예외 발생 , try ~ catch
            FileOutputStream fout = new FileOutputStream(path);
            // (3) 출력할 내용
            String str = "오늘 날씨는 맑았다. 자바 공부는 재밌다. ";
        ...
        try{
            ...
            // 3. 파일 경로 지정
            String path2 = "src/day16/visit_log.txt";
            // 4. 지정 경로에 파일 존재 여부 확인
           ...
           /* 방안1
            ...
            if( file2.exists() ) { // 파일 존재하면 // 입력
                // 5. 파일 입력 객체
                FileInputStream finput = new FileInputStream( path2 );
                // 6. 파일 용량 만큼 배열 선언
            ...
            inStr += outStr;    // 9. 읽어온 문자열 뒤로 새로운 문자 연결, 불러온 문자열과 입력받은 문자열 연결
