...
public class Example1 {
    public static void main(String[] args) {
        ...
        [ ArrayList ] 가변길이, 다양한 메소드 제공
            - 자바 회사에서 미리 제공하는 클래스
            1. 정의 : 배열과 다르게 컬렉션프레임워크 중 리스트 자료형 클래스
            2. 목적 : 가변길이의 배열을 지원
            3. 사용법
                1) 선언방법
                ...
                2) 요소 추가
                    ...
                    * 인덱스란 ? 배열 또는 리스트 내 저장된 (요소)값들의 저장순서번호, 0번시작
                3) 출력/반환
                    (1) 변수명     : 전체 요소값 문자열 반환
         ...
         */
        // [1] ArrayList 객체 생성하기
        ArrayList< String > 리스트객체 = new ArrayList<>(); // 자동생성해야 import가 된다.
        ...
        // [5] .size();     :리스트의 크기 확인하기 vs 배열에서 사용한 .length 와 비슷하다.
        System.out.println( 리스트객체.size() );

        // [6] .get()   : 요소값 출력/반환    vs   배열에서 사용한 변수명[인덱스]와 비슷하다.
        ...
        // (2) 향샹된 반복문  , for( 항목타입 변수명 : 리스트명 ) { }
        for ( String str : 리스트객체 ) { //리스트(목록) 내 요소값 하나씩 str 에 대입 반복
            System.out.println( str );
        }
        ...
        // 2) <Member> 타입/클래스의 객체 3개를 지정할 ArrayList 객체 생성
        ArrayList<Member> memberList = new ArrayList<>(); // < 꺽은괄호에는 리스트/목록 내 저장할 항목의 타입
        // 3) 리스트에 객체들을 저장하기
        memberList.add( m1 );
        memberList.add( m2 );
        memberList.add( m3 );
