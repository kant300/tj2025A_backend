...
public class Example2 {
    ...
    public static void main(String[] args) { // 공개 우선할당 리턴이 없다. 배열이 매개변수에 저장
        ...
        System.out.println( D.비멤버공유변수 );  // 객체없이 static 가능
        D.비멤버공유변수 = 20;
        System.out.println( d.비멤버공유변수 );  // 객체도 static 가능

        // [3] 상수 호출 , 클래스명.상수명;
        System.out.println( D.상수 );
        // [4] static 메소드 내 staric 유무 메소드 호출
        // 멤버메소드();   // 불가능 :
        ...
        /*
        [ final ] *최후의
            1. 수정불가능한/고정상태를 정의할때 사용하는 키워드
            2. 사용법 : final 타입 멤버변수
            3. 주의할점 : 초기값이 무조건 존재 해야 한다.
        [ static ] *정적인
            1. 프로그램이 실행될때 메모리가 할당되고 프로그램이 종료될때 메모리 삭제되는 키워드
               실행중에 변화가 없는 상태를 정의할때 사용되는 키워드
            2. 사용법 : static 타입 멤버변수 -------> static(정적)변수
            3. 주의할점 : 멤버변수(객체내변수) 공유X new   vs   static(우선할당)  공유O
                3-1. 프로그램내 딱 1개만 존재하는 변수를 선언할때 사용. 서로 다른 객체들이 공유해서 사용한다.
        [ 상수 ]  : 변하지 않는 변수
            1. 사용법 : static final 타입 상수명;
            2. 주의할 점 : 2-1.수정불가능  2-2.프로그램내 딱 1개 존재  2-3.상수명 대문자 사용
        ...
        class Test {
            ...
            void main2(){ }
            static void main(){
                main2(); // 불가능 : main함수는 우선할당이 되면서 main2가 누구세요?
                new Test().main2(); // 가능 :
