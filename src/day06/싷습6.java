...
public class 싷습6 {  //class s
    public static void main(String[] args) {    //main s
        ...
        System.out.println( numbers[2] ); // 배열의 2번 인덱스에 저장된 값을 콘솔에 출력

        // [2] 배열선언
        String[] season = new String[3]; //String 타입의 데이터를 3개 저장할 수 있는 배열 season을 new 키워드를 사용하여 생성
        ...
        // (방법2) 향샹된 for 문 , for( 배열타입 반복변수명 : 배열변수명 ) {}
        ...
        for( int score : scores2 ) {
            if (score == 100) { break; }   // 만약에 배열내 요소가 100점이면 반복문종료
        }   // for e
        ...
        //  [8] 향상된 for 문 사용하지 않은 이유. 재고의 index 필요해서
        String[] products = {"볼펜", "노트", "지우개"};
        int[] stock = {10, 5, 20};
        Scanner scan = new Scanner( System.in );    // 1) 입력객체 생성하여 변수에 저장
        System.out.println(">>상품명 : ");    String name = scan.next();  // 2) 상품명 입력받아 변수에 저장
        ...
        for( int index = 0 ; index <= products.length - 1 ; index++ ) { // 3) 0부터 마지막인덱스까지 1씩 반복
            ...
            if( product.equals( name) ) { // 5) 만약에 index번째 제품명과 입력받은 값이 같으면
                if( stock[index] >= count ) { // 6) 만약에 index번째 재고보다 입력받은 재고가 이하이면
        ...
        for( int index = 0 ; index <= movieNames.length - 1 ; index++ ){   // [영화이름]
            String movie = movieNames[index];   // index번째 영화이름 추출
            System.out.println( movie );    // index번째 영화이름 출력
