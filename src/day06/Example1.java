package day06;  //형대 .jabva파일의 폴더 위치
    ...
    public static void main(String[] args) {    // main s
        ...
        [ 배열 ]
            ...
            3. 사용법
                ...
                2) 호출
                    (1) 변수명         , 배열(객체)의 주소값
                    (2) Arrays.toString( 변수명 ); , 배열내 요소값 반환
                    (3) 변수명[인덱스]
                        * 인덱스란? 배열내 저장된 요소들의 순서번호, 0부터 시작
                        * 변수명.length : 배열내 요소개수 반환 , 배열의길이
                3) 수정
                    (1) 변수명 = new 타입[ 3 ];  , 전체 배열 수정
                    (2) 변수명[인덱스] = 새로운값;    , 특정 인덱스의 값 수정

                4) 삭제/추가 * 특정한 요소 삭제는 존재하지 않는다. 자바 배열은 *고정길이* 이므로 중간에 요소 총개수 변경할 수 없다.
                5) 배열과 반복문
                    (1)
                    for( int index = 0 ; index <= 배열변수명.length - 1 ; index++ ) {
                        타입 변수명 = 배열변수명[indedx];
                    }
                    (2) 향상딘 for 문 , 인덱스 없이 배열내 요소값 하나씩 변수에 대입 반복
                    for ( 타입 변수명 : 배열변수명 ) {
        ...
        // [3] 배열선의 주소값 반환
        System.out.println( array1 ); // [I@4c873330 :  객체의 조소(컴퓨터가 사용하는 메모리 위치정보) 값
