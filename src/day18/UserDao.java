...
public class UserDao {
    ...
    public void connect() {
        try {
            ...
            conn = DriverManager.getConnection(db_url, db_user, db_password); // 2. MYSQL DB 서버 연동
        } catch (ClassNotFoundException e) {
            System.out.println("[경고] MYSQL 라이브러리 코드 실패");
        } catch (SQLException e) {
            System.out.println("[경고] DB서버와 연동 실패");
        }
        // 대부분 오타 or db가 없거나 패스워드에 띄어쓰기면 빈칸도 패스워드에 반영
    }// func e

    // (*) 싱글톤 : 주로 프로그램내 하나의 객체(톤) 표현
