...
public class Example2 {
    public static void main(String[] args) {
        ...
        // 활용3] 아스키코드 패턴으로 난수 계산시 못만들 경우 직접 난수에 넣을 데이터를 배열로 선언
        ...
        System.out.println( "생성도니 패스워드 : " + newPwd );
        ...
        ArrayList< Integer > lottoList1 = new ArrayList<>();
        for(  ;  ;  ){
            int lotto = random.nextInt( 45 ) + 1 ; // 생성된 로또 번호
            if( lottoList.contains( lotto ) ){ // 리스트에 생성된 로또번호가 존재하면
               continue; // 이번 반복문 무효처리
            }
            lottoList1.add( lotto ); // 생성된 로또번호를 리스트에 담기
            if( lottoList.size() == 6 ) break;; // 로또번호가 6개이면 반복문 종료
        }
        System.out.println( lottoList );
        ...
        char str = ranStr2.charAt( ranIndex ); // (3) 문자열내 인덱스 위치한 문자1개 추출
        System.out.println( str );
