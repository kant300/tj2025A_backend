...
public class Example1 {
    public static void main(String[] args) {
        ...
        char str5 = '김';                        System.out.println( (int) str5 ); // 44608  //정수로 타입변환

        // 3. 문자열 리터럴 : " " 큰 따옴표 , String 클래스
        String str6 = "유재석";  //65번지 주소                      // - new String 사용하지 않고 문자열 생성할 경우
        String str7 = "유재석";  //65번지 주소                      // 문자열이 동일할 경우 동일한 객체 취급
        String str8 = new String( "유재석" ); //66번지 주소   // - new String 사용시 동일한 문자열 이라도 다른 객체 취급

        // 4.       ==                    vs      .equals(): 문자열비교메소드   ,
        // == *주소*값(객체가 저장된*위치*) 비교            객체내 *값* 비교
        ...
        System.out.println( "자바"+"프로그램밍");

        // 2) StringBuilder 클래스 : 문자열을 단계적으로 연결 하는 메소드 제공하는 클래스
        StringBuilder builder = new StringBuilder();
        builder.append("자바");
        builder.append("프로그램밍");
        System.out.println( builder );  // 자바프로그램밍  // 메모리 효율 좋다

        // 3) String.format( "형식문자" , "문자열A" );
        String str10 = String.format( "%s%s" , "자바" , "프로그래밍");
        System.out.println( str10 ); // 자바프로그램밍
        // 4) +연결연산자
        String str11 = "";
        str11 += "자바";
        str11 += "프로그램밍";
        System.out.println( str11 ); // 자바프로그램밍
        ...
        sql3 += "insert into ";
        sql3 += "table(name) ";
        sql3 += "values("+name+")"; // 비권장 : 앞 이나 뒤 한쪽은 띄어쓰기 주의
