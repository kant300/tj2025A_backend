...
public class Example1 {
    public static void main(String[] args) {
        ...
        /*
        인터페이스 : 추상메소드를 가진 타입
        추상메소드 : 선언만하고 구현부{}를 정의하지 않은 메소드
        구현체 : 추성메소드를 가진 인터페이스를 implements 해서 추상메소드를 구현한 클래스의 객체
        ** 다형성 ** : 다양한 타입을 가지는 성질 , extends / implements 키워드 구조관계
        ...
        // [2] List 인터페이스 주요 메소드
            // 1) .add( 자료 ) : 리스트내 지정한 자료에 마지막 요소 추가
        list2.add("유재석");
        list2.add("강호동");
        list2.add("신동엽");
        list2.add("유재석"); // 중복허용
        ...
        list2.set( 0 , "유재석2");
        System.out.println( list2 );
            // 3) .get( 인덱스 ) : 리스트내 지정한 인덱스의 자료를 호출/반환
        String result = list2.get( 2 ); //'서장훈'반환
        System.out.println( result );
            // 4) .size() : 리스트내 전체요소/자료 개수 반환
       ...
       // System.out.println( list2 );
        ...
        // (3) forEach 문
        // 리스트내 첫번째 요소부터 마지막 요소까지 순서대로 반복변수에 대입하여 실행처리한다.
        // 리스트/배열명.forEach()
        // list2.forEach( ( 반복변수) ) -> { 실행문}
        // 람다식/화살표 함수 : 복잡한 함수 선언 방식을 간소함 , ( 매개변수 ) -> { 실행문 }
        list2.forEach( ( str ) -> { System.out.println( str ); } );
        list2.forEach( ( str ) -> { System.out.println(list1); } );

        // ---------------------------------------------------------- //
        // List : 순서대로 요소 저장하는 자료구조
        // ArrayList : 배열구조로 사용되는 List 구현체
        ArrayList< Integer > arrayList = new ArrayList<>();
        // Vector : 배열구조로 사용되는 List 구현체 + 메소드가 동기화( 멀티 스레드 플랫폼 )
        Vector< Integer > vector = new Vector<>();
        // LinkedList : 링크(체인/꼬리연결) 구조로 사용되는 List 구현체 + 중간삽입/삭제 효율성 극대화
            // 배열구조 : [ 5 ] [ 4 ] [ 7 ] [ 10 ]
                //        0      1     2     3
                // 만약에 1번 인덱스가 삭제 되면 2/3 인덱스는 한칸씩 이동
                // 즉] 중간에 변화가 있으면 뒤에 요소들의 위치변화
            // 링크구조 : [ 5 ] -> [ 4 ] -> [ 7 ] -> [ 10 ]
                //    5/4주소갑 -> 4/7주소값 -> 7/10주소값 -> 10/주소값없다.
