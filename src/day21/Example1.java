...
public class Example1 {
    public static void main(String[] args) {
        ...
        [ 멀티 스레드 Multi_Thread ]
            1. 정의 : main스페드 외 새로운 *작업스레드*를 생성하여 동시 작업
        ...
        5. 주요 클래스/인터페이스
            ...
            (2) Runnable 인터페이스
                1) run 추상메소드 : 작업스레드가 실행하는 최조 시작점 처리 메소드
        ...
        Runnable runnable1 = new Runnable(){
            // run 추상메소드 구현
            @Override // run 추상메소드구현 // 오버라이딩 // 지정의
            public void run() {
                for( int i = 1; i<=5 ; i++ ) {
                    toolkit.beep();
                    try { Thread.sleep( 1000 ); }
        ...
        Thread thread1 = new Thread( runnable1 ); // run 구현한 runnable 타입을 Threac생성자에 대입
        thread1.start(); // run 구현된 runnable 타입을 갖는 thread 객체가 작없 스레드 .start() 실행
        ...
        Thread thread2 = new Thread( runnable1 ); // run 구현한 runnable 타입을 Threac생성자에 대입
        thread1.start(); // run 구현된 runnable 타입을 갖는 thread 객체가 작없 스레드 .start() 실행
        ...
        Thread thread3 = new Thread( 작업스레드1 ); // 구현체를 Thread 객체에 대입
