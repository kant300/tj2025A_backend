...
class  Calculator{
    ...
    // 메소드
    public synchronized void setMemory( int memory ){ // 두개 이상의 스레드가 해당 메소드 사용시 먼저 요청한 스레드가 점령하고 종료될때가지 다른 스레드는 await(대기)만든다.
   // public void setMemory( int memory ){
        this.memory = memory; //매개변수 대입
        try{ Thread.sleep(2000); } // 2초간 일시정시
        catch ( Exception e ){}
        System.out.println( this.memory ); // 멤버변수 확인
        // user1 스레드가 2초가 일시정지 도중에 user2 스레드가 200 으로 대입
        // 주의할점 : 두개 이상의 스레드가 하나의 메소드를 호출 할때 비동기 처리를 한다.
        // 동기화 : 두개 이상의 스레다가 하나의 메소드를 순서대로 호출하고 처리할때 사용한다.
        // 동기화 목적 : 두개 이상의 스레드가 먼저 메소드를 호출하고 다른 스레드는 await 상태로 만든다.
        // 예] 유제삭(thread1) 강호동(thread2) 신동엽(thread3) 가 달리기() 함수 실행한다.
...
public class Example3 {
    public static void main(String[] args) {
        ...
        [ synchronized ] 동기화
            ...
            2. 목적 : 여러 스레드가 동시에 하나의 스레드를 사용하면 멤버변수(코드) 충돌 생길수 있다.
            3. 비교
                동기화                              비동기화(기본값)
                한번에 하나의 스레드가 처리             여러개의 스레드가 동시 처리
                처리순서 보장                        처리순서 보장 안됨
                synchronized                        x
                주문순서대로 음료제조후 손님에게 준다     제조가 먼저 완성된 음료를 순서대로 손님에게 준다.


            4. 사용법
                (방법1) 메소드 선언부의 반환타입 앞에 'synchronized' 키워드 붙인다.
                (방버2) synchronized(this){} 블럭 이용한다.
        ...
        // 3. 두 스레드 실행
        user1.start(); // user1은 100넣기 : 200
        user2.start(); // user2은 200넣기 : 200
